...
class ListenChatAgent(ChatAgent):
    ...
    def _execute_tool(
        ...
        # Drop stray message_* params that ToolkitMessageIntegration injects
        # into every tool schema; non-MCP tools reject them as unexpected kwargs.
        args = {
            k: v
            for k, v in tool_call_request.args.items()
            if k
            not in ("message_title", "message_description", "message_attachment")
        }
    ...
    async def _aexecute_tool(
        ...
        # Graceful fallback when the model hallucinates a tool name that
        # isn't in our surface. Without this the dict lookup raises
        # KeyError and crashes the whole turn (observed with
        # 'list_repositories' hallucinated as a GitHub MCP tool).
